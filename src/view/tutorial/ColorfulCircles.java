...
public class ColorfulCircles extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		LinearGradient gradient = new LinearGradient(0f, 1f, 1f, 0f, true, CycleMethod.NO_CYCLE, gradientStops);
		...
		for (Node circle: circles.getChildren()) {
			...
			KeyValue endX = new KeyValue(circle.translateXProperty(), random() * 800);
			KeyValue endY = new KeyValue(circle.translateYProperty(), random() * 600);
			timeline.getKeyFrames().addAll(
					new KeyFrame(Duration.ZERO, startX, startY),
					new KeyFrame(new Duration(40000), endX, endY)
					);
		}
